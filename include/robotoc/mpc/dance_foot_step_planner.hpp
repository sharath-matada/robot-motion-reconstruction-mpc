#ifndef ROBOTOC_DANCE_FOOT_STEP_PLANNER_HPP_
#define ROBOTOC_DANCE_FOOT_STEP_PLANNER_HPP_
...
///
/// @class DanceFootStepPlanner

/// @brief Foot step planner for dance gait of quadrupeds. 
///
class DanceFootStepPlanner
 final : public ContactPlannerBase {
public:
  ///
  /// @brief Constructs the planner.
  /// @param[in] quadruped_robot Quadruped robot model. 
  ///
  DanceFootStepPlanner
  ...
  ///
  DanceFootStepPlanner
  & operator=(const DanceFootStepPlanner
  &) = default;
  ...
  ///
  /// @brief This is invalid in DanceFootStepPlanner
  ///
  ...
  friend std::ostream& operator<<(std::ostream& os, 
                                  const std::shared_ptr<DanceFootStepPlanner
                                  >& planner);
...
private:
  ...
  std::vector<Eigen::Vector3d> com_ref_, com_to_contact_position_local_, prev_contact_position_;
  std::vector<Eigen::Matrix3d> R_;
  Eigen::Vector3d vcom_, vcom_cmd_, step_length_;
  Eigen::Matrix3d R_yaw_, R_current_, R_yaw1_;
