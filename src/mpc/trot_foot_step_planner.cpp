...
TrotFootStepPlanner::TrotFootStepPlanner(const Robot& quadruped_robot)
  ...
  if (quadruped_robot.maxNumPointContacts() < 4) {
    throw std::out_of_range(
        "[TrotFootStepPlanner] invalid argument: 'robot' is not a quadrupedal robot!\n robot.maxNumPointContacts() must be larger than 4!");
  }
...
void TrotFootStepPlanner::setRaibertGaitPattern(const Eigen::Vector3d& vcom_cmd,
                                                ...
                                                const double gain) {
  if (swing_time <= 0.0) {
    throw std::out_of_range("[TrotFootStepPlanner] invalid argument: 'swing_time' must be positive!");
  }
  if (stance_time < 0.0) {
    throw std::out_of_range("[TrotFootStepPlanner] invalid argument: 'stance_time' must be non-negative!");
  }
  if (gain <= 0.0) {
    throw std::out_of_range("[TrotFootStepPlanner] invalid argument: 'gain' must be positive!");
  }
...
const aligned_vector<SE3>& TrotFootStepPlanner::contactPlacements(const int step) const {
  throw std::runtime_error("[TrotFootStepPlanner] runtime error: contactPlacements() is not implemented!");
